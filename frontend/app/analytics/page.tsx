...
export default function Analytics() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.get(`http://192.168.0.137:5000/analytics?jobs=${jobName}`)
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      <h1 className="text-5xl font-bold text-sky-600 text-center my-10">CareerFit AI</h1>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <ul className="list-disc list-inside mb-4">
                {analyticsData.skill_demand.emerging_skills.map((skill, index) => (
                  <li key={index}>{skill}</li>
                ))}
              </ul>
              <h3 className="font-semibold mb-2">High Demand Skills</h3>
              <ul className="list-disc list-inside">
                {analyticsData.skill_demand.high_demand_skills.map((skill, index) => (
                  <li key={index}>{skill}</li>
                ))}
              </ul>
            </CardContent>
            ...
            <CardContent>
              <ul className="list-disc list-inside">
                {analyticsData.top_hiring_companies.map((company, index) => (
                  <li key={index}>
                    <strong>{company.company_name}</strong> - {company.description}
                  </li>
                ))}
              </ul>
            </CardContent>
